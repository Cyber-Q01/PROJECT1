...
export default function RegisterPage() {
  ...
  useEffect(() => {
    ...
    // Max birth date is 5 years ago from today
    const fiveYearsAgo = subYears(today, 5);
    // Min birth date is Jan 1, 1950
    const minBirthDateInstance = new Date(1950, 0, 1); // Month is 0-indexed

    setCalendarDateConfig({
        defaultMonth: fiveYearsAgo, // Default month to show in calendar
        fromDate: minBirthDateInstance,  // Earliest selectable date
        toDate: fiveYearsAgo,        // Latest selectable date
    });
  }, []);
  ...
  const handleFormSubmit: SubmitHandler<RegistrationFormValues> = async (data) => {
    ...
    const studentDataToSubmit = {
      ...data,
      dateOfBirth: formatISO(data.dateOfBirth), // Store as ISO string
    ...
    // Exclude id as it's generated by MongoDB
    ...
    try {
      ...
      // Use the ID from the API response (MongoDB _id)
      const newRegistrationDataForState: StoredStudentData = {
        id: result.student.id || result.student._id.toString(), // Ensure we get the string ID
    ...
    try {
      ...
      try {
        ...
        result = { 
          ...
          details: `Status: ${response.status}. Response: ${responseText.substring(0, 250)}...` // Limit length of raw text in toast
        }; 
      }
